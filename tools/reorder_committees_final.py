...
def process_section(start_regex, end_regex, approved_order):
    ...
    for i in range(1, len(blocks)):
        ...
        if i == len(blocks) - 1:
            a_idx = block.rfind('</a>')
            if a_idx != -1:
                close1 = block.find('</div>', a_idx)
                if close1 != -1:
                    close2 = block.find('</div>', close1 + 6)
                    if close2 != -1:
                        split_point = close2 + 6
                        trailing_html = block[split_point:]
                        block = block[:split_point]
        ...
        if name_match:
            ...
            trailing_html = block + trailing_html
...
t_start, t_end, t_new = process_section(r'>Technical Program Committee members</h3>', r'<div data-people-group>\s*<h3[^>]*>Finance Chairs</h3>', tech_order)
